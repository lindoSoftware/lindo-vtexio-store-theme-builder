...
export class GroupCardProcessor extends CustomPageBlockProcessor<ComponentSharedGroupCard> {
  process(
    ...
    const groupRow = this.generateBlockName(
      'flex-layout.row',
      `group-card-${index}`
    )
    const groupCol = this.generateBlockName(
      'flex-layout.col',
      `group-card-${index}`
    )
    ...
    if (section.name) {
      const titleBlock = this.generateBlockName(
        'rich-text',
        `group-card-title-${index}`
      )
    ...
    section.cards.forEach((card, cardIndex) => {
      const cardRow = this.generateBlockName(
        'flex-layout.row',
        `group-card-${index}-card-${cardIndex}`
      )
    ...
    content.forEach((item, itemIndex) => {
      if (item.appName === 'CardTextBlock') {
        const textBlock = this.generateBlockName(
          'rich-text',
          `group-${groupIndex}-card-${cardIndex}-text-${itemIndex}`
        )
        ...
        item.images.forEach((image: any, imgIndex: number) => {
          const imageBlock = this.generateBlockName(
            'image',
            `group-${groupIndex}-card-${cardIndex}-img-${itemIndex}-${imgIndex}`
          )
