...
exports.getOtelInstruments = void 0;
const client_1 = require("./client");
const createOtelConcurrentRequestsInstrument = async () => {
    const metricsClient = await (0, client_1.getMetricClient)();
    return metricsClient.createGauge('io_http_requests_current', {
        description: 'The current number of requests in course.',
        unit: '1'
    });
};
const createOtelRequestsTimingsInstrument = async () => {
    const metricsClient = await (0, client_1.getMetricClient)();
...
const createOtelTotalRequestsInstrument = async () => {
    const metricsClient = await (0, client_1.getMetricClient)();
...
const createOtelRequestsResponseSizesInstrument = async () => {
    const metricsClient = await (0, client_1.getMetricClient)();
...
const createOtelTotalAbortedRequestsInstrument = async () => {
    const metricsClient = await (0, client_1.getMetricClient)();
...
class OtelInstrumentsSingleton {
    ...
    async initializeInstruments() {
        const [concurrentRequests, requestTimings, totalRequests, responseSizes, abortedRequests] = await Promise.all([
...
const getOtelInstruments = () => OtelInstrumentsSingleton.getInstance().getInstruments();
